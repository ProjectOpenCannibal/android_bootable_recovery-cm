...
void COTSettings::CreateOrSaveSettings(int is_new) {
	if (is_new == 1) {
		...
		iniparser_set(COTSettings::settingsini, "settings:zip_sigverif", zip_sigverif.string());
...
void COTSettings::LoadSettings() {
	...
	zip_sigverif = iniparser_getstring(COTSettings::settingsini, "settings:zip_sigverif", NULL);
